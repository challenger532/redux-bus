...
exports.default = function (middlewares, store, next, action, current) {
  ...
  if (middleware) {
    ...
    queue = middleware(store, next, action, queue, current);
...
var _undoLastAction = require('./presets/undo-last-action');
...
var _holdActions = require('./presets/hold-actions');
...
var _networkHandler = require('./presets/network-handler');

Object.defineProperty(exports, 'network', {
  enumerable: true,
  get: function get() {
    return _interopRequireDefault(_networkHandler).default;
  }
});
...
},{"./middleware":3,"./presets/hold-actions":5,"./presets/network-handler":6,"./presets/undo-last-action":7,"./reducer":8}],3:[function(require,module,exports){
'use strict';
  ...
  return function (store) {
    return function (next) {
      return function (action) {
        ...
        bus = (0, _parse2.default)(bus);

        if (!bus) return next(action);

        action.bus = bus;

        return manage(middlewares, store, next, action, bus);
      };
    };
  };
};

var manage = function manage(middlewares, store, next, action, bus) {

  if (_is_js2.default.json(bus)) {
    delete action.bus;
    if (action.payload) delete action.payload.bus;
    return (0, _handle2.default)(middlewares, store, next, action, bus);
  }

  if (_is_js2.default.array(bus)) {

    if (_is_js2.default.empty(bus)) {
      delete action.bus;
      if (action.payload) delete action.payload.bus;
      return next(action);
    }

    var portion = bus.shift();

    if (!bus.length) delete action.bus;
    if (_is_js2.default.json(portion)) {
      return (0, _handle2.default)(middlewares, store, next, action, portion);
    }

    if (_is_js2.default.array(portion)) {
      if (!_is_js2.default.empty(bus)) portion.forEach(function (item, i) {
        (0, _handle2.default)(middlewares, store, next, action, item);
      });
      action.bus = bus;
      return;
    }
...
},{"./handle":1,"./parse":4,"is_js":9}],4:[function(require,module,exports){
'use strict';
...
exports.default = function (bus) {
  if (!bus) return;
  if (_is_js2.default.string(bus)) {
    var portions = bus.split(/\s*\|\s*/).reduce(function (busses, portion) {
      var portion_busses = portion.split(/\s*\&\s*/).reduce(function (result, item) {
        item = item.trim();
        var words = item.split(/\s+/);
        if (words.length == 2) {
          var handler = words[0].trim().toLowerCase();
          var command = words[1].trim().toLowerCase();
          if (_is_js2.default.not.empty(handler) && _is_js2.default.not.empty(command)) {
            result.push({
              handler: handler,
              command: command
            });
      ...
      if (_is_js2.default.not.empty(portion_busses)) {
        if (portion_busses.length == 1) portion_busses = portion_busses.shift();
        busses.push(portion_busses);
      }
      return busses;
    }, []);
    if (_is_js2.default.not.empty(portions)) {
      if (portions.length == 1) portions = portions.shift();
    }
    return portions;
  }
...
},{"is_js":9}],5:[function(require,module,exports){
'use strict';
...
exports.default = function (store, next, action, queue, bus) {
  var temp = void 0;
  switch (bus.command) {
    case 'push':
...
exports.default = function (store, next, pass, action, queue, bus) {
